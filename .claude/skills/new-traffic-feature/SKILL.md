...
**這個 repo 是公開的。** 完整規則在 `CLAUDE.md` 的「資料機敏性檢查」一節，
新增功能時最容易踩到的是這三個：

1. **測試與範例一律用合成資料。** 想不出手算案例就自己編一份小樣本
   （見 `tests/test_motc_su.py` 的 `SAMPLE`），**不要**把真實案件的軌跡檔、
   影像或客戶規格書加進 repo。需要真實檔案驗證時，請使用者提供本機路徑，
   跑完把數字寫進工作單就好，檔案留在本機。
2. **提交前列出會被送出去的新檔案**（`git status --short --untracked-files=all`），
   逐一確認沒有真實資料、客戶文件、生產環境程式、個資、本機絕對路徑、
   或數 MB 以上的二進位檔。命中就停下來問使用者：
   加進 `.gitignore`／改寫成不含實資料的版本／確認可公開。
3. **`.gitignore` 對已經被追蹤的檔案無效。** 順手比對一次
   「已追蹤但已列入 ignore」的檔案；發現已提交的機敏資料時，
   把 `CLAUDE.md` 那張表的三個選項（停止追蹤／改寫內容／重寫歷史）
   連同代價一起講清楚，讓使用者決定，不要自己動手。
...
- 不要動 `data/` 裡的舊版參考程式。
- 不要把真實案件的資料或客戶文件加進版控——`data/` 已列入 `.gitignore`，
  不要為了跑測試而把它加回來。
